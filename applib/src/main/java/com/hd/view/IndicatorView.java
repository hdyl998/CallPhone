...
/**
 * Created by gyzhong on 15/2/13. Viewpager 页卡指示器，封装了指示器的切换 和生成过程，可以根据viewPager
 * 的 pager count 来生成指示灯的 个数，并且通过Viewpager 的 viewPager 偏移量来动态的改变指示灯 的位置，和当前选中的
 * view
 */
public class IndicatorView extends View implements ViewPager.OnPageChangeListener {
    ...
    /*
     * 因为ViewPager 的 pageChangeListener 被占用了，所以需要定义一个 以便其他调用
     */
    ...
    /**
     * 测量View 的大小，这个方法我前面的 b//log 讲了很多了，
     *
     ...
     * @param viewPager
     */
    public void setDatas(ViewPager viewPager) {
        if (viewPager == null) {
            return;
        }
        ...
        setDatas(viewPager, pagerAdapter.getCount());
    }


    public void setDatas(ViewPager viewPager, int realCount) {
        if (viewPager == null) {
            return;
        }
        mCount = realCount;
        viewPager.addOnPageChangeListener(this);
        mSelectItem = getRealPositon(viewPager.getCurrentItem());
        invalidate();
    }
    ...
    public void onPageScrolled(int position, float positionOffset, int positionOffsetPixels) {
        ...
        if (mSmooth) {
            mSelectItem = getRealPositon(position);
